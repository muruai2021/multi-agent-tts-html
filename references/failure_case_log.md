...
| F-021 | 2026-06-19 | Phase 0.3 | v1.5.3 体检发现：`SubsGen.py` Step 2 短句合并用 `len(buffer) < min_chars` 算字符总数（含标点），但 SKILL.md 文档说"中文字符数 ≥ 10 字"。结果 9 字自然句 + 1 句号 = 11 字符（≥10）通过硬约束不被合并，导致 27/39 字幕 < 10 字（69% 违规率） | ①加 helper `_cn_count(s) = len(re.findall(r'[一-鿿]', s))` 统一中文字数②替换 Step 2/3/4/5 全部 `len()` 阈值判断为 `_cn_count()`③加 Step 3.5 短段回合并（循环到全合规）④加 Step 6 末段吸附⑤wx-agent-ai 案例回归：27/39 违规 → 0/21 合规（100%） | v1.5.4 |
